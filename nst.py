...
def parse_args():
    ...
    parser.add_argument('--content', type=str, default='images/tubingen.jpg', help='File path of content image (notation in the paper : p)', required = False)
    parser.add_argument('--style', type=str, default='images/starry-night.jpg', help='File path of style image (notation in the paper : a)', required = False)
    parser.add_argument('--output', type=str, default='result.jpg', help='File path of output image', required = False)
...
def deleteFile(fileName):
	if os.path.isfile(fileName):
		...
		print("delete error: %s file not found" % fileName)

def transfer(content,style,result,args):	
	# initiate VGG19 model
	model_file_path = args.model_path + '/' + vgg19.MODEL_FILE_NAME
	vgg_net = vgg19.VGG19(model_file_path)

	# load content image and style image
	content_image = utils.load_image(content, max_size=args.max_size)
	...
	# save result
	utils.save_image(result_image,result)
...
def main():
	...
	### bot
	#get new task
	data=mySqlQuery("""SELECT user,content,style FROM trans WHERE content!='' AND style!='' ORDER BY id ASC LIMIT 1""")
	for rec in data:
		user, content, style = rec
		print (user, content, style)
	...
	if len(data):
		...
		#message to user
		url = messageUrl+'style transfer in progress..'
		requests.get(url)

		#download content
		url = 'http://scriptlab.net/telegram/bots/transtylebot/'+content
		download_file(url,content)

		#download style
		url = 'http://scriptlab.net/telegram/bots/transtylebot/'+style
		download_file(url,style)

		#delete the task	
		mySqlQuery("""DELETE FROM trans WHERE user = '"""+user+"""'""")
		
		#delete result
		#deleteFile(result)
		
		transfer(content,style,result,args)
		
		#send result
		url = 'http://scriptlab.net/telegram/bots/transtylebot/sendResult.php?user='+user+'&result='+result+'&content='+content+'&style='+style
		requests.get(url)
